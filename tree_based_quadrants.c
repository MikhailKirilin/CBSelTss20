...
int
is_strict_ancestor (const quadrant_t * q, const quadrant_t * r)
{
  if (is_valid (q) && is_valid (r)) {
    return is_ancestor(q, r) && !is_equal(q, r);
  }
  else {
    printf ("is_strict_ancestor: the input quadrants have to be valid.\n");
    return -1;
  }
}
