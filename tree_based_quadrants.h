...
/** Test if a quadrant is a strict ancestor of another quadrant.
 * \param [in] q Quadrant to be tested.
 * \param [in] r Descendent quadrant.
 * \return true if \a q is a strict ancestor of \a r, that is, an ancestor that is unequal to \a r.
 *          ( -1) if there was an error
 */
int                 is_strict_ancestor (const quadrant_t * q, const quadrant_t * r);
